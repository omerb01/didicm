...
This script is used to validate a model on the ImageNet dataset using the Discrete Diffusion Classification Modeling framework
proposed in the paper "Advancing Image Classification with Discrete Diffusion Classification Modeling" by Omer Belhasin.
The validation script was adapted from the ImageNet Validation Script in the timm library.
"""
import argparse
import csv
...
parser.add_argument('--output', default='', type=str, metavar='FILENAME',
...
parser.add_argument('--diffusion-enabled', action='store_true', default=False,
                   help='Enable diffusion')
parser.add_argument('--diffusion-sampler', type=str, default='cp',
                   help='Sampler for diffusion')
parser.add_argument('--diffusion-sampler-kwargs', nargs='*', default={}, action=ParseKwargs,
                   help='Keyword arguments for diffusion sampler')
parser.add_argument('--diffusion-noise', type=str, default='loglinear',
                   help='Noise type for diffusion')
parser.add_argument('--diffusion-steps', type=int, default=8,
                   help='Number of diffusion steps')
parser.add_argument('--diffusion-eps', type=float, default=1e-5,
...
def validate(args):
    ...
    if args.diffusion_enabled:
        generation_record["all_data_probs_steps"] = []
    ...
    with torch.inference_mode():
        ...
        if not args.naflex_loader:
            ...
            if args.diffusion_enabled:
                label_cond = torch.randint(0, args.num_classes, (args.batch_size,)).to(device=device)
                time_cond = torch.randn((args.batch_size,)).to(device=device, dtype=model_dtype)
                with amp_autocast():
                    model(input, c=label_cond, t=time_cond)
            else:
                model(input)
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                if args.diffusion_enabled:
                    output, output_steps = diffusion_sampler.run(model, input)
                    output = output.to(device=device, dtype=model_dtype)
                    generation_record["all_data_probs"].append(output.cpu())
                    generation_record["all_data_probs_steps"].append(output_steps.cpu())
                else:
                    output = model(input)
                    generation_record["all_data_probs"].append(output.cpu())
                generation_record["all_data_targets"].append(target.cpu())
    ...
    for key, value in generation_record.items():
        generation_record[key] = torch.cat(value, dim=0)

    # save generation_record to pt
    if args.output:

        if args.diffusion_enabled:
            diffsuion_args = {
                "diffusion_noise_type": args.diffusion_noise_type,
                "diffusion_predictor": args.diffusion_predictor,
                "diffusion_steps": args.diffusion_steps,
                "diffusion_eps": args.diffusion_eps,
                "diffusion_sampler": args.diffusion_sampler,
                "diffusion_sampler_kwargs": args.diffusion_sampler_kwargs,
            }
            with open(os.path.join(args.output, "diffusion_args.json"), "w") as f:
                json.dump(diffsuion_args, f)
        
        torch.save(generation_record, os.path.join(args.output, "generation_record.pt"))
